	...
	int solver)
{
	...
	if (solver == 0) // Point correspondence-based fundamental matrix estimation
		...
		num_inl = findFundamentalMatrixAC_(
			...
			sampler_variance,
			lo_number);
	else if (solver == 3) //  4p4d fundamental matrix estimation
		num_inl = findFundamentalMatrix4p4d_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 4) //  4p3d fundamental matrix estimation
		num_inl = findFundamentalMatrixPT4D3_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 5) //  3p3d fundamental matrix estimation
		num_inl = findFundamentalMatrixPT3D3_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 6) //  6-point fundamental matrix estimation
		num_inl = findFundamentalMatrixPT6_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 7) // Monodepth equal focal fundamental matrix estimation GB solver
		num_inl = findFundamentalMatrixMonoEqualGB_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 8) // Monodepth equal focal fundamental matrix estimation Eigen solver
		num_inl = findFundamentalMatrixMonoEqualEigen_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 9) // Monodepth varying focal fundamental matrix estimation GB solver
		num_inl = findFundamentalMatrixMonoVarGB_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 10) // Monodepth varying focal fundamental matrix estimation Eigen solver
		num_inl = findFundamentalMatrixMonoVarEigen_(
			correspondences,
			probabilities,
			inliers,
			F,
								...
								int solver)
{
	...
	if (solver == 0) // Point correspondence-based fundamental matrix estimation
		...
		num_inl = findEssentialMatrixAC_(
			...
			sampler_variance,
			lo_number);
	else if (solver == 3) // Relative depth 3DV 2024
		num_inl = findEssentialMatrixRel_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 4) // Monodepth 3PT suv closed-form
		num_inl = findEssentialMatrixMono_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 5) // inverse depth model
		num_inl = findEssentialMatrixInverse_(
			correspondences,
			probabilities,
			inliers,
			F,
			...
			sampler_variance,
			lo_number);
	else if (solver == 6) // P3P
		num_inl = findEssentialMatrixP3P_(
			correspondences,
			probabilities,
			inliers,
			F,
...
PYBIND11_PLUGIN(pygcransac) {
  ...
  m.def("findHomography", &findHomography, R"doc(some doc)doc",
	...
	m.def("solver_homography_two_affine", &solver_homography_two_affine, R"doc(some doc)doc",
        py::arg("correspondences"));

	m.def("solver_homography_two_sift", &solver_homography_two_sift, R"doc(some doc)doc",
        py::arg("correspondences"));

	m.def("solver_homography_four_point", &solver_homography_four_point, R"doc(some doc)doc",
        py::arg("correspondences"));
