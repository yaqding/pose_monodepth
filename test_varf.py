...
from errors import calc_mAA_pose, essential_pose_error_focalvar
import pygcransac
...
def run_gcransac(pair, matches, relative_pose, image_size1, image_size2, K1, K2, args):
    ...
    if args.solver == 0 or args.solver == 6:
        ...
        matches[:,0] = (matches[:,0] - K1 [0,2])/scalar
        matches[:,1] = (matches[:,1] - K1 [1,2])/scalar
    ...
    elif args.solver == 3 or args.solver == 4 or args.solver == 9 or args.solver == 10:
        # The point correspondences
        matches[:,0] = (matches[:,0] - K1 [0,2])/scalar
        matches[:,1] = (matches[:,1] - K1 [1,2])/scalar
        matches[:,2] = (matches[:,2] - K2 [0,2])/scalar
    ...
    if inlier_number >= 4:  
        ...
        rotation_error, translation_error, focal_error = essential_pose_error_focalvar(E_est, scalar, relative_pose, K1, K2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--solver", type=str, help="Choose from: point, sift, affine.", choices=["point", "sift", "affine", "4p4d", "4p3d", "4ptvarG", "4ptvarE"], default="4ptvarG")
