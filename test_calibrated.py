...
from errors import calc_mAA_pose, essential_pose_error
import pygcransac
...
def run_gcransac(pair, matches, relative_pose, image_size1, image_size2, K1, K2, args):
    ...
    elif args.solver == 3 or args.solver == 4 or args.solver == 5 or args.solver == 6 or args.solver == 7:
        # The point correspondences
        points = matches[:, :4]
    ...
    if correspondences.shape[0] < 8:
        return 0, rotation_error, translation_error, inlier_number, numofmatches

    # Run the essential matrix estimation implemented in gcransac
    ...
    if inlier_number >= 3:  
        ...
        # Calculate the pose error of the estimated essential matrix given the ground truth relative pose
        rotation_error, translation_error = essential_pose_error(E_est, relative_pose)


    return runtime, rotation_error, translation_error, inlier_number, numofmatches
...
def estimate_essential(pairs, data, args):
    ...
    # Run essential matrix estimation on all image pairs
    ...
    return times, rotation_errors, translation_errors, inlier_numbers, nmatches
...
if __name__ == "__main__":
    ...
    parser.add_argument("--solver", type=str, help="Choose from: point, sift, affine.", choices=["point", "6pt","sift", "affine", "rel", "mono", "p3p", "M100", "inverse", "5ori", "6pt"], default="sift")
    ...
    for scene in configuration[f'{split}_SCENES']:
        ...
        times, rotation_errors, translation_errors, inlier_numbers, nmatches = estimate_essential(pairs, data, args)
